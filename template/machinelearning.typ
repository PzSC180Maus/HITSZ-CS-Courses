...
#definition[无偏学习 (Unbiased Learning)][
  无偏学习指学习算法*不预设任何归纳偏置*，即对所有假设一视同仁，仅通过纯粹的观察数据进行学习。

  具体来说，无偏学习器对所有与训练数据一致的假设赋予相同的权重，不做任何偏向性选择。
]

#theorem[无偏学习的无用性定理][
  一个完全没有归纳偏置的学习算法无法进行有效的泛化。

  *原因*：
  1. 对于未见实例 $x'$，所有与训练数据一致的假设可能给出完全不同的预测
  2. 无偏学习器无法在这些不一致的预测中做出选择
  3. 因此无偏学习器对未见实例的预测是任意的、无意义的

  *本质原因*：
  - 归纳学习本质上是"从有限样本推断一般规律"
  - 有限的训练数据永远无法唯一确定目标概念
...
#solution[
  *无偏学习的定义*：
  无偏学习指算法不预设任何归纳偏置，不对任何假设有偏好。

  *无偏学习的无用性*：
  - 当面对未见过的测试实例时，所有与训练数据一致的假设中，有些预测为正类，有些为负类
  - 无偏学习器没有选择依据，无法确定应该采用哪个假设的预测
  - 因此无法对未见实例做出有意义的预测
...
#solution[
  *(1) 错误率降低修剪（Reduced-Error Pruning）*

  *策略步骤*：
  1. 先用训练集构建一棵完整的决策树
  2. 用验证集评估每个内部节点（子树）替换为叶子节点后的分类错误率
  3. 若替换为叶子节点后验证集错误率不上升（或下降），则执行剪枝，将子树替换为叶子节点
  4. 重复直到进一步剪枝会提高错误率
  ...
  *缺点*：
  - 需要额外的验证集，减少了训练数据量
  - 剪枝结果依赖于验证集的质量和规模
  - 可能会有"延迟效果"——一个看起来无用的子树可能在更深层有重要子节点

  *(2) 规则后修剪（Rule Post-Pruning）*

  *策略步骤*：
  1. 先从完整决策树中提取分类规则（每个叶子节点到根节点的路径为一条规则）
  2. 对每条规则进行剪枝：移除不导致规则准确率下降的前提条件
  3. 对规则排序后应用于新实例
...
#example[2024期末考题 - 决策树与信息增益计算（10分）][
  ...
  #align(center)[
    #table(
      ...
      table.header([序号], [Shape], [Color], [Odor], [Edible]),
      [1], [C], [B], [1], [Yes],
      [2], [D], [B], [1], [Yes],
      [3], [D], [W], [1], [Yes],
      [4], [D], [W], [2], [Yes],
...
#solution[
  ...
  *总数据集*：11个样本，Yes=5, No=6

  总熵：
  $ 
  "Entropy"(S) = -5/11 log_2 5/11 - 6/11 log_2 6/11
  = -0.455 times (-1.137) - 0.545 times (-0.875)
  = 0.517 + 0.477 = 0.994
  $

  Shape 属性取值：C（5个样本：1,5,8,9,10 → Yes=1, No=4），D（6个样本：2,3,4,6,7,11 → Yes=4, No=2）

  各子集熵：
  - Shape=C：$H(S_C) = -1/5 log_2 1/5 - 4/5 log_2 4/5 = 0.464 + 0.257 = 0.721$
  - Shape=D：$H(S_D) = -4/6 log_2 4/6 - 2/6 log_2 2/6 = 0.389 + 0.528 = 0.917$

  加权平均熵：
  $ "Entropy"(S, "Shape") = 5/11 times 0.721 + 6/11 times 0.917 = 0.328 + 0.500 = 0.828 $

  信息增益：
  $ "Gain"(S, "Shape") = 0.994 - 0.828 = 0.166 $
...
#solution[
  *神经元基本模型*
  ...
  神经元（Perceptron/M-P神经元）是人工神经网络的基本计算单元，模拟生物神经元的"兴奋"和"抑制"机制。

  *数学表示*：
  1. 接收 $n$ 个输入信号 $x_1, x_2, ..., x_n$
  2. 每个输入乘以对应的权重 $w_1, w_2, ..., w_n$
  3. 求加权和再加上偏置（阈值）：$z = sum_(i=1)^n w_i x_i + b$
  4. 通过激活函数 $f$ 输出：$y = f(z)$

  *常见激活函数*：
  - 阶跃函数（Step）：$y = cases(1 "if" z > 0, -1 "otherwise")$
  - Sigmoid：$y = 1 / (1 + e^(-z))$
  - ReLU：$y = max(0, z)$
...
#solution[
  *1. 感知器法则（Perceptron Rule）*
  - 适用于线性可分的单层感知器
  - 权值更新公式：$Delta w_i = eta (t - o) x_i$
    其中 $t$ 为目标输出，$o$ 为实际输出（$o in {+1, -1}$）
  - 只在 $t != o$（即分类错误）时更新权重
  - 理论保证：如果数据线性可分，感知器法则经过有限次迭代必然收敛

  *2. Delta法则（Delta Rule / LMS / Widrow-Hoff Rule）*
  - 适用于任何可微激活函数的神经元（包括非线性）
  - 权值更新公式：$Delta w_i = eta (t - y_"in") x_i$
    其中 $y_"in"$ 为线性输出 $y_"in" = sum w_i x_i$（而非经过阶跃函数后的输出）
  - 使用梯度下降最小化误差平方和：$E(bold(w)) = 1/2 sum (t - y_"in")^2$
  - 无论数据是否线性可分，Delta法则都能收敛到最小二乘解
  ...
  *3. 关键区别*
  ...
  #align(center)[
    #table(
      ...
      inset: 5pt, align: center,
      table.header([对比项], [感知器法则], [Delta法则]),
      [更新依据], [分类是否正确（$t != o$）], [线性输出误差（$t - y_"in"$）],
      [适用激活], [阶跃函数（不可微）], [任意可微函数],
      [收敛条件], [数据线性可分], [凸误差函数],
      [优化方式], [在线学习], [梯度下降],
      [收敛效果], [找到分离超平面], [最小化MSE],
      [延续发展], [SVM（最大间隔分离超平面）], [逻辑回归（交叉熵）],
    )
  ]
]
...
#solution[
  *1. Find-S 算法步骤*
  Find-S 算法从最特殊假设开始，每遇到一个正例就将其泛化：
  - Step 1：将 $h$ 初始化为最特殊假设 $<∅, ∅, ..., ∅>$
  - Step 2：对每个正例 $x$，对于 $h$ 的每个属性约束：
    - 若该属性约束与 $x$ 一致，不做修改
    - 若不一致，将该属性约束改为 $?$（接受任意值）
  - Step 3：输出最终假设 $h$
  *特点*：只考虑正例，忽略反例；输出假设一定是与正例一致的最特殊假设。

  *2. 决策树规则后修剪步骤*
  - Step 1：从完整决策树中提取分类规则（每条路径对应一条规则）
  - Step 2：对每条规则进行剪枝——移除不提高规则准确率的条件
  - Step 3：将规则排序，按优先级应用于新实例
  - Step 4：当出现分类冲突时由排序最高的规则决定

  *3. 朴素贝叶斯分类器计算公式*
  $ h_"NB"(x) = "argmax"_(y in cal(Y)) P(y) product_(i=1)^d P(x_i | y) $
  其中：
  - $P(y)$ 是类别 $y$ 的先验概率
  - $P(x_i | y)$ 是在给定类别 $y$ 下属性 $x_i$ 的条件概率
  - 关键假设：各属性在给定类别下条件独立

  *4. 特征子集搜索的三种方式*
  - *前向搜索（Forward Search）*：从空集开始，逐步添加使评价函数最优的特征
  - *后向搜索（Backward Search）*：从全集开始，逐步剔除使评价函数最优的特征
  - *双向搜索（Bidirectional Search）*：同时从前向和后向开始搜索，直到相遇
  三种方式都是贪心策略，只能保证局部最优。
]
...
#solution[
  ...
  *两层网络结构*：
  - *输入层*：接收 $D$ 维输入向量 $x$
  - *隐藏层*：$k$ 个 RBF 神经元，每个以 $mu_i$ 为中心计算 $x$ 到中心的径向距离，通过高斯核输出
  - *输出层*：线性加权求和，$hat(f)(x) = sum w_i phi_i(x)$
...
#solution[
  遗传算法（Genetic Algorithm）的算法流程如下（伪代码形式）：

  ```
  Algorithm: 遗传算法
  
  Input: 适应度函数f, 种群大小N, 交叉概率Pc, 变异概率Pm
  Output: 最优解
  
  1. 初始化：随机生成N个个体的初始种群P(0)
  2. t = 0
  3. REPEAT:
  4.     评估：计算P(t)中每个个体的适应度
  5.     选择：根据适应度选择N个父代个体
  6.     交叉：以概率Pc对父代进行交叉操作
  7.     变异：以概率Pm对子代进行变异操作
  8.     生成新一代种群P(t+1)
...
#exam[2025期末考题 - 线性回归最小二乘法（10分）][
  ...
  (2) 根据表中数据，用最小二乘法求出 $y$ 关于 $x$ 的线性回归方程 $y = b x + a$（8分）
]

#solution[
  *(1) 散点图描述*：
  - 横轴为记忆力 $x$，纵轴为判断力 $y$
  - 四个点：$(6,2), (8,3), (10,5), (12,6)$
  - 大致呈正相关线性趋势
  ...
  *验证*：
  - $x = 6$：$0.7 times 6 - 2.3 = 4.2 - 2.3 = 1.9 approx 2$
  - $x = 12$：$0.7 times 12 - 2.3 = 8.4 - 2.3 = 6.1 approx 6$
]
...
#exam[2024期末考题 - K-Means聚类计算（10分）][
  ...
  即样本点为：$(0,2), (1,0), (5,0), (5,2), (0,0)$

  距离定义为欧氏距离，初始聚类中心为 $C_1 = (0,2)$ 和 $C_2 = (0,0)$，聚类簇个数为 2。

  要求：给出每一步的聚类结果（每次聚类要写明每个聚类包含的样本及聚类中心）。
]

#solution[
  *初始化*：$mu_1 = (0, 2)$，$mu_2 = (0, 0)$

  *第1轮分配*：
  - 样本 $(0,2)$：到 $mu_1$ 距离 $0$，到 $mu_2$ 距离 $2$ → 归入簇1
  - 样本 $(1,0)$：到 $mu_1$ 距离 $sqrt(5) approx 2.24$，到 $mu_2$ 距离 $1$ → 归入簇2
  - 样本 $(5,0)$：到 $mu_1$ 距离 $sqrt(29) approx 5.39$，到 $mu_2$ 距离 $5$ → 归入簇2
  - 样本 $(5,2)$：到 $mu_1$ 距离 $5$，到 $mu_2$ 距离 $sqrt(29) approx 5.39$ → 归入簇1
  - 样本 $(0,0)$：到 $mu_1$ 距离 $2$，到 $mu_2$ 距离 $0$ → 归入簇2

  *第1轮聚类结果*：
  - 簇1：${(0,2), (5,2)}$，新质心 $mu_1' = (2.5, 2)$
  - 簇2：${(1,0), (5,0), (0,0)}$，新质心 $mu_2' = (2, 0)$

  *第2轮分配*：
  - $(0,2)$：到 $mu_1'$ 距离 $sqrt(6.25) approx 2.5$，到 $mu_2'$ 距离 $sqrt(8) approx 2.83$ → 簇1
  - $(1,0)$：到 $mu_1'$ 距离 $sqrt(6.25) approx 2.5$，到 $mu_2'$ 距离 $1$ → 簇2
  - $(5,0)$：到 $mu_1'$ 距离 $sqrt(10.25) approx 3.2$，到 $mu_2'$ 距离 $3$ → 簇2
  - $(5,2)$：到 $mu_1'$ 距离 $2.5$，到 $mu_2'$ 距离 $sqrt(13) approx 3.61$ → 簇1
  - $(0,0)$：到 $mu_1'$ 距离 $sqrt(10.25) approx 3.2$，到 $mu_2'$ 距离 $2$ → 簇2

  *第2轮聚类结果*：分配未发生变化，算法收敛。
